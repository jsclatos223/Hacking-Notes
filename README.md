...
## Dynamo's Prepnote Script (Modified)
```bash
#!/bin/bash
...
    destination="$OBSIDIAN_NOTES_PATH/OffSec/OSCP Exam"
...
    destination="$OBSIDIAN_NOTES_PATH/OffSec/OSCP Training"
    boxes_destination_base="$TRAINING_PATH"
    machine_prefix=("AD_Set" "Challenge1" "Challenge2" "Challenge3")
    ad_subfolders=("MS01" "DC01" "MS02")
elif [[ "$ad_flag" == true ]]; then
    destination="$OBSIDIAN_NOTES_PATH/OffSec/OSCP - AD Challenge Labs"
    boxes_destination_base="$OSCP_AD_PATH"
    machine_prefix=("Machine1" "Machine2" "Machine3")
elif [[ "$oscp_flag" == true ]]; then
    destination="$OBSIDIAN_NOTES_PATH/OffSec/OSCP - Challenge Labs"
    boxes_destination_base="$OSCP_PATH"
elif [[ "$proving_flag" == true ]]; then
    destination="$OBSIDIAN_NOTES_PATH/OffSec/Proving Grounds"
    boxes_destination_base="$PG_PATH"
elif [[ "$hackthebox_flag" == true ]]; then 
    destination="$OBSIDIAN_NOTES_PATH/HackTheBox/Machines"
    boxes_destination_base="$HTB_PATH"
else
    destination="$OBSIDIAN_NOTES_PATH/Other"
    boxes_destination_base="$HTB_PATH"
    echo "No specific flag provided. Defaulting to 'Other'."
fi
